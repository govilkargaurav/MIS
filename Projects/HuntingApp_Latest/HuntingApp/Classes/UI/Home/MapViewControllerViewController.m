...
@synthesize timeLine;
@synthesize tableViewNotification;
@synthesize mapView,isCurrentViewMapView;
@synthesize delegate;
@synthesize arrPostNotificationImage;
@synthesize view1;
@synthesize view2;
static int count = 0;
...
- (void)viewDidLoad
{
    ...
    favLocController = [[FavLocTableViewController alloc]initWithStyle:UITableViewStyleGrouped andItemsArray:favLocations];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    tableViewNotification.hidden=YES;
}

- (void)viewDidAppear:(BOOL)animated
{
    
    NSLog(@"%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"launchOptions"]);
    
    [super viewDidAppear:animated];
    if (shouldReload)
        [self setUpTimelineView];
    
//    NSString *strpush =[[NSUserDefaults standardUserDefaults] objectForKey:@"PUSHIMAGE"];
//    
//        if ([strpush isEqualToString:@"1"]) {
//    
//            [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"PUSHIMAGE"];
//            AppDelegate *delegate2 = (AppDelegate*)[[UIApplication sharedApplication] delegate];
//            delegate2.delegate=self;
...
-(void)picturePUSH:(NSString *)strID{
    
    likeSelected = NO;
    commentSelected = NO;
    selectedImage = [[DAL sharedInstance]getImageByImageID:strID];
        ...
        else if (button.tag == 1)
        {
            if (isCurrentViewMapView)
            {
                ...
                if(favLocController)
                {
                    ...
                    section = favLocController.selectedSection;
                    RELEASE_SAFELY(favLocController);
                }
                ...
                [favLocController setSelectedSection:section];
...
{
    if (tableView==tableViewNotification) {
        return [[[arrPostNotificationImage objectAtIndex:0] valueForKey:@"ALERT"] count];
    }else{
...
{
    NSString *CellIdentifier = [[NSString stringWithFormat:@"%d %d",indexPath.row,indexPath.section] retain];
    celltimeline = [[tableView dequeueReusableCellWithIdentifier:CellIdentifier] retain];
...
//        cell.textLabel.font = [UIFont fontWithName:@"WOODCUT" size:10.0];
//        cell.textLabel.numberOfLines=0;
//        cell.textLabel.textColor=[UIColor whiteColor];
//       // cell.textLabel.text = [NSString stringWithFormat:@"%@",[[[arrPostNotificationImage objectAtIndex:0] valueForKey:@"ALERT"] objectAtIndex:indexPath.row]];
//        return cell;
    }else{
        
        if(celltimeline == nil) {
            
            celltimeline = [[TimeLineCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
            //This shouldn't happen but you can build manually to be safe.
            celltimeline.backgroundView = [[[UIView alloc] initWithFrame:CGRectZero] autorelease];
        }
            ...
            if (index < picArray.count)
            {
                ...
                FLButton *imgBtn = [[FLButton alloc] initWithFrame:CGRectMake(5, 0, 145, 190)];
                [celltimeline.btn1 setHidden:NO];
                ...
                if (!pic.image)
                {
                   ...
                   [imgBtn setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",pic.image_url]] placeholderImage:[UIImage imageNamed:@"loading.png"] options:SDWebImageCacheMemoryOnly];
                    //[imgBtn setImageWithImage:pic];
                }
                [celltimeline addSubview: imgBtn];
                ...
                CustomTimeLineFooter *view = celltimeline.footer1;
                view.frame = CGRectMake(-5,191,150,20);
                [celltimeline.footer1 setHidden:NO];
                ...
                [celltimeline  addSubview:view];
                index ++;
            }
            else
            {
                [celltimeline.btn1 setHidden:YES];
                [celltimeline.footer1 setHidden:YES];
            }
            if (index < picArray.count)
            {
                Picture *pic = [picArray objectAtIndex:index];
                ...
                FLButton *imgBtn = [[FLButton alloc] initWithFrame:CGRectMake(162,0,145,190)];
                [celltimeline.btn2 setHidden:NO];
                [imgBtn.imageView getWhiteBorderImage];
                if (!pic.image)
                {
                    [imgBtn setImageWithImage:pic];
                ...
                CustomTimeLineFooter *view = celltimeline.footer2;
                [celltimeline.footer2 setHidden:NO];
                [view.btnName setContentHorizontalAlignment:UIControlContentHorizontalAlignmentLeft];
                NSString *username = [[pic.user_desc componentsSeparatedByString:@":"]lastObject];
                username = [[username componentsSeparatedByString:@" "] objectAtIndex:0];
                [view.btnName setTitle:username forState:UIControlStateNormal];
                ...
                [celltimeline.btn2 setHidden:YES];
                [celltimeline.footer2 setHidden:YES];
            }
            return celltimeline;
        }
