...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(reportAbuse:) name:REPORT_ABUSE object:nil];
    ...
    if (!pic.album)
    {
        [imgUsrProfile setImageWithURL:[NSURL URLWithString:[Utility getProfilePicURL:[[pic.user_desc componentsSeparatedByString:@":"]objectAtIndex:0]]] placeholderImage:[UIImage imageNamed:@"Unknown-person.png"]];
        ...
        [btnProfileImg setTag:[[[pic.user_desc componentsSeparatedByString:@":"]objectAtIndex:0]intValue]];
    }
    else
    {
        ...
        if (!pic.album.user_profile.picture)
            ...
            [imgUsrProfile setImageWithURL:[NSURL URLWithString:[Utility getProfilePicURL:[[pic.user_desc componentsSeparatedByString:@":"]objectAtIndex:0]]] placeholderImage:nil];
...
//            [profilePic setImage:[UIImage imageWithData:pic.album.user_profile.picture] forState:UIControlStateHighlighted];
        ...
        [btnProfileImg setTag:pic.album.user_profile.user_id.intValue];
    }
...
-(void)reportAbuse:(id)sender{
    if ([MFMailComposeViewController canSendMail]){
        ...
        NSArray *toRecipients = [[NSArray alloc]initWithObjects:@"OutdoorLoop@gmail.com", nil];
        [mailer setSubject:@"Report abuse.."];
        [mailer setToRecipients:toRecipients];
        [self presentViewController:mailer animated:YES completion:nil];
        
    }else{
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Outdoorloop"
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)ascrollView
{
    if (ascrollView == scrollVw)
    {
        ...
        if (!pic.album)
        {
            ...
            [imgUsrProfile setImageWithURL:[NSURL URLWithString:[Utility getProfilePicURL:[[picture.user_desc componentsSeparatedByString:@":"]objectAtIndex:0]]] placeholderImage:[UIImage imageNamed:@"loading.png"]];
            ...
            [btnProfileImg setTag:[[[picture.user_desc componentsSeparatedByString:@":"]objectAtIndex:0]intValue]];
        }
        else
        {
            NSLog(@"%@",[NSURL URLWithString:[Utility getProfilePicURL:[[picture.user_desc componentsSeparatedByString:@":"]objectAtIndex:0]]]);
...
//                [profilePic setImage:[UIImage imageWithData:picture.album.user_profile.picture] forState:UIControlStateHighlighted];
            ...
            [btnProfileImg setTag:picture.album.user_profile.user_id.intValue];
        }
...
- (IBAction)comment:(id)sender 
{
    if (!isCommentViewLoading)
    {
        ...
        if ([Utility userID])
        {
            ...
            UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"Outdoorloop" message:@"OOPS..\nIt seems you are not Logged In. Please make Login first to see Likes and Comment." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
            [alert show];
...
-(void)gotoProfilePUSH:(NSString *)sender{
    ...
    Profile *profile = [[DAL sharedInstance] getProfileByID:sender];
