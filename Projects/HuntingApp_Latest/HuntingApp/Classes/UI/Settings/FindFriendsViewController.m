...
- (void)requestWrapperDidReceiveResponse:(NSString *)response withHttpStautusCode:(HttpStatusCodes)statusCode withRequestUrl:(NSURL *)requestUrl withUserInfo:(NSDictionary *)userInfo
{
    ...
    {
        NSLog(@"%lu",(unsigned long)[[JSONDict objectForKey:@"data"] count]);
        NSLog(@"%@",[JSONDict objectForKey:@"data"]);
        ...
        [store requestAccessToAccountsWithType:twitterType options:nil completion:^(BOOL granted, NSError *error){
            if (granted) {
                ...
                if (twitterAccounts.count!=0)
                {
                    ...
                    __block SLRequest *request  = [SLRequest requestForServiceType:SLServiceTypeTwitter
                                                                     requestMethod:SLRequestMethodGET
                                                                               URL:[NSURL URLWithString:[NSString stringWithFormat:@"https://api.twitter.com/1.1/friends/ids.json"]]
                                                                        parameters:nil];
...
//                         }
                         ...
                         if (![ids isEqualToString:@""])
                         {
                             ...
                             NSDictionary *_dictUserIDs = [NSDictionary dictionaryWithObject:ids forKey:@"user_id"];
                             __block SLRequest *request2  = [SLRequest requestForServiceType:SLServiceTypeTwitter
                                                                               requestMethod:SLRequestMethodPOST
                                                                                         URL:[NSURL URLWithString:[NSString stringWithFormat:@"https://api.twitter.com/1.1/users/lookup.json"]]
                                                                                  parameters:_dictUserIDs];
                    ...
                    BlackAlertView *alert = [[[BlackAlertView alloc] initWithTitle:@"Find Friends" message:@"No Friends Found" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] autorelease];
                    [alert show];
...
- (NSArray *)getContactsList
{
    ...
    if (ABAddressBookGetAuthorizationStatus() == kABAuthorizationStatusNotDetermined) {
        ...
        ABAddressBookRequestAccessWithCompletion(addressBookRef, ^(bool granted, CFErrorRef error) {
            ...
            NSArray *arrTemp=(NSArray *)ABAddressBookCopyArrayOfAllPeople(addressBookRef);
