...
- (void) dealloc
{
	...
	DLog( @"TSAlertView: TSAlertOverlayWindow dealloc" );
...
- (void) dealloc
{
	DLog( @"TSAlertView: TSAlertViewController dealloc" );
	[super dealloc];
}
...
- (void) show
{
	...
	[self sizeToFit];
	self.center = CGPointMake( CGRectGetMidX( avc.view.bounds ), CGRectGetMidY( avc.view.bounds ) -100);;
	self.frame = CGRectIntegral( self.frame );
	[self pulse];
