...
-(void)loadView {
  ...
  // Nav bar button
    ...
    self.navigationController.navigationBar.tintColor = RGBCOLOR(62, 146, 0);
    
  if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7.0"))
    {
        
        UIButton *rightBarButtonItem = [[UIButton alloc] initWithFrame: CGRectMake(0, 0, 60.0f, 30.0f)];
        UIImage *okImage = [UIImage imageNamed:@"ok.png"];
        [rightBarButtonItem setBackgroundImage:okImage forState:UIControlStateNormal];
        
        [rightBarButtonItem addTarget:self action:@selector(didFinish) forControlEvents:UIControlEventTouchUpInside];
        UIBarButtonItem *backButtonItem = [[UIBarButtonItem alloc] initWithCustomView:rightBarButtonItem];
        self.navigationItem.rightBarButtonItem = backButtonItem;
        
        UIButton *leftBarButtonItem  = [[UIButton alloc] initWithFrame: CGRectMake(0, 0, 60.0f, 30.0f)];
        UIImage *cancel = [UIImage imageNamed:@"cancel.png"];
        [leftBarButtonItem  setBackgroundImage:cancel forState:UIControlStateNormal];
        
        [leftBarButtonItem  addTarget:self action:@selector(didCancel) forControlEvents:UIControlEventTouchUpInside];
        UIBarButtonItem *cancelButton = [[UIBarButtonItem alloc] initWithCustomView:leftBarButtonItem ];
        self.navigationItem.leftBarButtonItem = cancelButton;
  
        
      //  self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"ok.png"] style:UIBarButtonItemStyleBordered target:self  action:@selector(didFinish)];
      //  self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"cancel.png"] style:UIBarButtonItemStyleBordered  target:self action:@selector(didCancel)];
    }
  else
  {
...
//    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7.0"))
//        self.tableView.frame = CGRectMake(0,self.tableView.frame.origin.y+44, f.size.width, f.size.height - 40);
...
-(void)didFinish {
  // Delegate callback
  //if ([delegate respondsToSelector:@selector(didFinishSelectionWithItems:)]) {
