...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error 
{
    ...
    DisplayAlertWithTitle(APP_Name, @"Opps ..There is a problem in the internet connection!!!");
...
-(void)DonePressed
{
    ...
    if (settag == 1)
    {
        tfOccu.text = [NSString stringWithFormat:@"%@",[[ArryOccupation objectAtIndex:catID] valueForKey:@"vOccupation"]];
    }
    else if (settag == 2)
    {
        tfLoca.text = [NSString stringWithFormat:@"%@",[ArryLocation  objectAtIndex:catID]];
    }
...
-(IBAction)btnSearchPressed:(id)sender
{
    ...
    [tfLoca resignFirstResponder];
    [tfOccu resignFirstResponder];
    MateSearchViewCtr *obj_MateSearchViewCtr = [[MateSearchViewCtr alloc]initWithNibName:@"MateSearchViewCtr" bundle:nil];
    obj_MateSearchViewCtr.strName = [NSString stringWithFormat:@"%@",tfName.text];
    obj_MateSearchViewCtr.strOccupation = [NSString stringWithFormat:@"%@",tfOccu.text];
    obj_MateSearchViewCtr.strLocation = [NSString stringWithFormat:@"%@",tfLoca.text];
    [self.navigationController pushViewController:obj_MateSearchViewCtr animated:YES];
    obj_MateSearchViewCtr = nil;
    tfName.text = @"";
    tfLoca.text = @"";
    tfOccu.text = @"";
}
...
-(void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    if (textField==tfOccu) {
        ...
        occuPationTableView.frame=CGRectMake(34,252,255,125);
    }
    else
    {
        occuPationTableView.tag=1;
        occuPationTableView.frame=CGRectMake(34,303,255,118);
    }
}
    ...
    if (textField==tfOccu)
    {
        ...
        for(NSDictionary *s in ArryOccupation) {
            ...
            if ([string isEqualToString:@""]) {
                ...
                NSString *strCC = [tfOccu.text substringWithRange:NSMakeRange(0, textField.text.length-1)];
        ...
        if(textField==tfLoca){
            ...
            for(NSDictionary *s in ArryLocation) {
                ...
                if ([string isEqualToString:@""]) {
                    ...
                    NSString *strCC = [tfLoca.text substringWithRange:NSMakeRange(0, textField.text.length-1)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath 
{
    if (tableView.tag==0) {
        ...
        tfOccu.text=[[tableData valueForKey:@"vOccupation"] objectAtIndex:indexPath.row];
        
    }else 
        if (tableView.tag==1) {
            
            tfLoca.text=[[tableData valueForKey:@"vlocation"] objectAtIndex:indexPath.row];
