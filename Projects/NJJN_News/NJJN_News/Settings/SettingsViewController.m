...
-(void)CallOnOffPushNoti
{
    ...
    NSString *str_DeviceToken = [NSString stringWithFormat:@"%@c=user&func=notificationswitchlite&vDeviceToken=%@&vUdid=%@&value=%@",WebURL,[[NSUserDefaults standardUserDefaults] objectForKey:@"deviceToken"],[[NSUserDefaults standardUserDefaults] valueForKey:@"DeviceUDID"],strNotiOnOff];
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *urlresponse, NSData *response, NSError *error)
     {
         ...
         else
         {
             ...
             {
                 [self performSelector:@selector(PassNoti:) withObject:[dicResult valueForKey:@"eStatus"] afterDelay:0.1f];
             }
             else
             {
                 [AppDel dohideHUD];
...
-(void)PassNoti:(NSString*)str
{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:str forKey:@"NotificationOnOff"];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
