...
@interface EditReminderViewCtr ()

@end

@implementation EditReminderViewCtr
@synthesize strReminderID;
...
- (void)viewDidLoad
{
  ...
  //  AppDel.deckController.panningMode = IIViewDeckNoPanning;
    ...
    scl_bg.contentSize = CGSizeMake(320, 620);
    ...
    // Get Reminder For Update
    if ([strReminderID intValue] != 0)
    {
        ...
        if ([ArryStan_Reminder count] > 0)
        {
            for (int i = 0; i < [ArryChannel count]; i++)
            {
                if ([[[ArryChannel objectAtIndex:i]valueForKey:@"channelId"] intValue] == [[[ArryStan_Reminder objectAtIndex:0]valueForKey:@"beforeDueDateChannelId"] intValue])
                {
                ...
                if ([[[ArryChannel objectAtIndex:i]valueForKey:@"channelId"] intValue] == [[[ArryStan_Reminder objectAtIndex:0]valueForKey:@"onDueDateChannelId"] intValue])
                {
            ...
            strAllDaysCheckUncheck = [NSString stringWithFormat:@"%@",[[ArryStan_Reminder objectAtIndex:0]valueForKey:@"isEveryDay"]];
            tfReminderCountBeforeDay.text = [NSString stringWithFormat:@"%@",[[ArryStan_Reminder objectAtIndex:0]valueForKey:@"startDay"]];
        }
    }
    else
    {
    ...
    [self FillValueInReminder];
    
    
    // Do any additional setup after loading the view from its nib.
}
-(void)FillValueInReminder
{
...
-(IBAction)btnSavReminderPressed:(id)sender
{
    ...
    else
    {
        ...
        [[NSNotificationCenter defaultCenter]postNotificationName:@"GetReminderDataForEdit" object:dic];
        [self CallPopViewCtr];
    }
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self performSelector:@selector(dismissAlertView:) withObject:alertChannleList afterDelay:0.3];
...
-(void)updateui
{
    if ((self.interfaceOrientation==UIInterfaceOrientationPortrait) || (self.interfaceOrientation==UIInterfaceOrientationPortraitUpsideDown))
    {
        if([UserLanguage isEqualToString:@"de"])
        {
            lblSendReminder.frame = CGRectMake(32, 154, 163, 21);
