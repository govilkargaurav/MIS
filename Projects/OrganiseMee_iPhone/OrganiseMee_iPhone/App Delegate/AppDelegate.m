...
{
    NSArray *vList = [_navigationController viewControllers];
    UIViewController *view;
    for (int i=[vList count]-1; i>=0; --i)
    {
        view = [vList objectAtIndex:i];
        if ([view.nibName isEqualToString: @"HomeViewCtr"] || [view.nibName isEqualToString:@"HomeViewCtr_landscap"])
        {
            [_navigationController popToViewController:view animated:YES];
            break;
        }
    }
...
- (void)applicationWillEnterForeground:(UIApplication *)application
{
   ...
   // [NSObject cancelPreviousPerformRequestsWithTarget:_delegate selector:@selector(CallSyncFromBGTask) object:nil];
    NSString *loginStatus = [[NSUserDefaults standardUserDefaults] stringForKey:@"Login"];
	if ([loginStatus isEqualToString:@"LoggedIn"])
	{
...
// Global PopOver MEthod
#pragma mark - IBAction Methods
-(IBAction)btnGlobalBackPressed:(id)sender
{
