...
+(NSDictionary*)GetContactName:(NSString*)strAssignId TaskCategoryType:(NSString*)strType
{
    ...
    NSDictionary *mainDict = [ NSDictionary dictionaryWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"language" ofType:@"plist"]];
    NSMutableDictionary *localizationDict = [[NSMutableDictionary alloc] init];
    ...
    else
    {
        ...
        if ([ArryContact count] > 0)
        {
            ...
            if ([strAssignedToYouFlag isEqualToString:@"YES"])
            {
                ...
                {
                     strResponsible = [NSString stringWithFormat:@"Sie von %@ %@",strFName,strLName];
                }else{
