...
@interface TaskSettingViewCtr ()

@end

@implementation TaskSettingViewCtr
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	if([UserLanguage isEqualToString:@"de"])
    {
        ...
        localizationDict = [AppDel.langDict objectForKey:@"SettingTaskVC"];
        ...
        lblDisplayDueDate.text = [NSString stringWithFormat:@"%@",[localizationDict valueForKey:@"lblDisplayDueDate"]];
        lblDisplayPri.text = [NSString stringWithFormat:@"%@",[localizationDict valueForKey:@"lblDisplayPri"]];
        lblDisplayReminder.text = [NSString stringWithFormat:@"%@",[localizationDict valueForKey:@"lblDisplayReminder"]];
        lblDisplayResponsible.text = [NSString stringWithFormat:@"%@",[localizationDict valueForKey:@"lblDisplayResponsible"]];
    }
    
    scl_bg.contentSize = CGSizeMake(scl_bg.frame.size.width, 381);
    
    // Set Selected Task
    NSString *strDoNow = [[NSUserDefaults standardUserDefaults] stringForKey:@"TaskSetting"];
...
-(IBAction)btnYesOrCancelPressed:(id)sender
{
    if ([sender tag] == 11)
    {
        ...
        [[NSUserDefaults standardUserDefaults] setObject:strDoNow forKey:@"TaskSetting"];
