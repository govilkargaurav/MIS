...
@synthesize suggestionsListed = _suggestionsListed;
...
- (UITableView *)initWithTextField:(UITextField *)textField inViewController:(UIViewController *) parentViewController withOptions:(NSDictionary *)options
{
    ...
    if (self.tag==100)
    {
        frame = CGRectMake(23.0,79.0+iOS7ExHeight*2.0,190.0,165+iPhone5ExHeight);
    }
    else if (self.tag==100)
    {
        frame = CGRectMake(40.0,79.0+iOS7ExHeight*2.0,190.0,165+iPhone5ExHeight);
    }
    else
    {
        frame = CGRectMake(40,79+iOS7ExHeight,240,165+iPhone5ExHeight-iOS7ExHeight);
    }
    ...
    UIView *v = [[UIView alloc] initWithFrame:CGRectMake(0, 0,240, 1)];
...
- (BOOL) substringIsInDictionary:(NSString *)subString
{
    ...
    for (int i=0; i<[self.suggestionsDictionary count]; i++)
    {
        ...
        range =[[NSString stringWithFormat:@"%@",[[self.suggestionsDictionary objectAtIndex:i] objectForKey:@"locationname"]] rangeOfString:subString options:NSCaseInsensitiveSearch];
        
        if (range.location != NSNotFound && range.location==0)
        {
           [tmpArray addObject:[self.suggestionsDictionary objectAtIndex:i]]; 
        }
    }
    ...
    if ([tmpArray count]>0)
    {
        ...
        self.suggestionsListed=tmpArray;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [[self.suggestionOptions objectAtIndex:indexPath.row] objectForKey:@"locationname"];
    
    cell.backgroundView.backgroundColor=[UIColor colorWithWhite:0.0 alpha:0.3];
...
{
    [self.textField setText:[[self.suggestionOptions objectAtIndex:indexPath.row] objectForKey:@"locationname"]];
...
-(void)showSuggestionsForString:(NSString *)strSuggestion
{
    if (![strSuggestion length])
    {
        [self hideOptionsView];
        return;
    }
    else if ([self substringIsInDictionary:strSuggestion])
    {
        [self showOptionsView];
        [self reloadData];
    }
