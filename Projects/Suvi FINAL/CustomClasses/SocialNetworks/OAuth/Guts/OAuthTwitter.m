...
- (void) synchronousRequestTwitterTokenWithCallbackUrl:(NSString *)callbackUrl {
   	NSString *url = (socialnwtype==1)?OAUTH_TWITTER_REQUEST_TOKEN_URL:OAUTH_TUMBLR_REQUEST_TOKEN_URL;
...
{
    NSString *url = (socialnwtype==1)?OAUTH_TWITTER_ACCESS_TOKEN_URL:OAUTH_TUMBLR_ACCESS_TOKEN_URL;
    ...
    else 
    {
		...
		for (NSString *component in responseBodyComponents) {
			...
			NSArray *subComponents = [component componentsSeparatedByString:@"="];
            [oauthdictionary setObject:[subComponents objectAtIndex:1] forKey:[subComponents objectAtIndex:0]];
			//[self setValue:[subComponents objectAtIndex:1] forKey:[subComponents objectAtIndex:0]];
		}
		
        NSString *access_token=[NSString stringWithFormat:@"%@",[oauthdictionary objectForKey:@"oauth_token"]];
        NSString *access_token_secret=[NSString stringWithFormat:@"%@",[oauthdictionary objectForKey:@"oauth_token_secret"]];
        ...
        [socialdict setObject:access_token forKey:@"twitter_token"];
        [socialdict setObject:access_token_secret forKey:@"twitter_secrete"];
        [socialdict setObject:@"Authenticate" forKey:@"twitter"];
        
        [[NSUserDefaults standardUserDefaults]setObject:socialdict forKey:@"USER_DETAIL"];
        [[NSUserDefaults standardUserDefaults]synchronize];
        
        
        NSString  *urlstring1= [NSString stringWithFormat:@"%@%@",kAuthoriseUnauthoriseSocialNw,[NSString stringWithFormat:@"&func=auth_unauthTwitter&userID=%@&twitter_token=%@&twitter_secrete=%@",[[[NSUserDefaults standardUserDefaults]valueForKey:@"USER_DETAIL"]valueForKey:@"admin_id"],access_token,access_token_secret]];
