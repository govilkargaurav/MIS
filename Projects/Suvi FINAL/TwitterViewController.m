...
//    tblview.frame=CGRectMake(0,48.0+, 320.0,tblview.frame.size.height);
    ...
    [request performRequestWithHandler:^(NSData *responseData, NSHTTPURLResponse *urlResponse, NSError *error)
     {
         if ([urlResponse statusCode] == 200)
         {
             ...
             @try {
                 NSError *error;
                 NSDictionary *dicttwdata = [NSJSONSerialization JSONObjectWithData:responseData options:0 error:&error];
                 NSMutableArray *arrtwdata=[[NSMutableArray alloc]initWithArray:[dicttwdata objectForKey:@"users"]];
...
-(void)invitetwitterfollower:(id)sender
{
    ...
    selectedfollowerindex=[sender tag];
    [self showHUDLoader];
