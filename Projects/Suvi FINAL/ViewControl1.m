...
@interface ViewControl1 () <MKMapViewDelegate>
{
    BOOL isExpand;
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(updateNotificationCOUNT:) name:@"updateNotificationCOUNT" object:[[NSUserDefaults standardUserDefaults]objectForKey:@"LaunchOption"]];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(hideNotificationCOUNT) name:@"hideNotificationCOUNT" object:nil];
    ...
    if ([[[NSUserDefaults standardUserDefaults]valueForKey:@"SignUP"] isEqualToString:@"SignUPFirstTime"])
    {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:APP_Name message:@"Invite your friends from Contacts, Facebook and Twitter!" delegate:self cancelButtonTitle:@"Later" otherButtonTitles:@"Now", nil];
        alert.tag = 100;
        [alert show];
        return;
    }
...
-(void)getNoti
{
    if ([[[NSUserDefaults standardUserDefaults]objectForKey:@"LaunchOption"] objectForKey:@"UIApplicationLaunchOptionsRemoteNotificationKey"])
    {
        ...
        [self updateNotificationCOUNTDict:dictNotificationReceiver];
        [[NSUserDefaults standardUserDefaults]removeObjectForKey:@"LaunchOption"];
    }
}
-(void)updateNotificationCOUNTDict:(NSDictionary *)dict
{
    NSString *strNotiCount = [NSString stringWithFormat:@"%@",[dict valueForKey:@"total_badges"]];
    ...
    if([[self removeNull:strNotiCount] integerValue]!=0)
    {
        ...
        [[NSNotificationCenter defaultCenter]postNotificationName:@"showWinkCount" object:dict];
    }
    else
    {
...
-(void)updateNotificationCOUNT:(NSNotification *)notif
{
...
-(void)updateInitialTable
{
    ...
    if ([[[MyAppManager sharedManager] arrAllFeeds] count]==0)
    {
...
-(void)customiseNavBar
{
    ...
    [btnOption setFrame:CGRectMake(0, 0+iOS7ExHeight, 54, 45)];
    ...
    [btnContact setFrame:CGRectMake(211, 0+iOS7ExHeight, 54, 45)];
...
-(void)updateInviteCount
{
    ...
    if([[self removeNull:strREQCount] integerValue]!=0)
    {
        ...
        PendingRequestCountBadge.tag=150150;
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [self performSelector:@selector(getNoti) withObject:nil afterDelay:1];
...
-(void)loadFeeds
{
    ...
    else
    {
        ...
        [urlRequest setURL:[NSURL URLWithString:getFeedsALL]];
    ...
    @try {
        ...
        if ([[[MyAppManager sharedManager] fileManager] fileExistsAtPath:filePath])
        {
            ...
            for (int i=0; i<[arrBGPosts count]; i++)
            {
                ...
                //[[[MyAppManager sharedManager] arrAllFeeds] addObject:myFeed];
            }
        }
        ...
        for (int i =0; i<[finalDataArray count]; i++)
        {
            ...
            else if([myFeed.vType_of_content isEqualToString:@"wrote"])
            {
                ...
                myFeed.wroteOn_id =[NSString stringWithFormat:@"%@",[self removeNull:[dict objectForKey:@"iAdminID"]]];
            ...
            {
                myFeed.strAttributed=[NSString stringWithFormat:@"%@%@%@%@", myFeed.generalText,([myFeed.vImWithflname isEqualToString:@""])?@"":[NSString stringWithFormat:@" with %@",myFeed.vImWithflname],([myFeed.vIamAt isEqualToString:@""])?@"":[NSString stringWithFormat:@" - at %@",myFeed.vIamAt],([myFeed.vIamAt2 isEqualToString:@""])?@"":[NSString stringWithFormat:@" %@",myFeed.vIamAt2]];
            }
            else
            {
                myFeed.strAttributed=[NSString stringWithFormat:@"%@%@%@%@", myFeed.vActivityText,([myFeed.vImWithflname isEqualToString:@""])?@"":[NSString stringWithFormat:@" with %@",myFeed.vImWithflname],([myFeed.vIamAt isEqualToString:@""])?@"":[NSString stringWithFormat:@" - at %@",myFeed.vIamAt],([myFeed.vIamAt2 isEqualToString:@""])?@"":[NSString stringWithFormat:@" %@",myFeed.vIamAt2]];
        ...
        {
            [[[MyAppManager sharedManager] arrAllFeeds]  addObject:[[temp objectAtIndex:i] objectForKey:@"feed"]];
        }
        
        //[[[MyAppManager sharedManager]arrAllFeeds] removeAllObjects];
        
        //[[MyAppManager sharedManager] sortMyFeeds];
...
-(IBAction)AddComment:(id)sender
{
    ...
    ViewFeed *myFeed = [[[MyAppManager sharedManager] arrAllFeeds] objectAtIndex:selectedIndexPath];
    ...
    [dict setValue:@"0" forKey:@"isFromHome"];
...
{
    ViewFeed *myFeed = [[[MyAppManager sharedManager] arrAllFeeds] objectAtIndex:[sender tag]];
...
#pragma mark - STATUS & IMAGE METHODS
...
-(void)setData:(NSDictionary*)dictionary
{
    ...
    if([self.action isEqualToString:@"_deleteHandler"])
    {
        ...
        if ([strMSG isEqualToString:@"SUCCESS"]) 
        {
            [[[MyAppManager sharedManager] arrAllFeeds] removeObjectAtIndex:selectedIndexPath];
...
{
    return [[[MyAppManager sharedManager] arrAllFeeds] count];
}
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ViewFeed *myFeed = [[[MyAppManager sharedManager] arrAllFeeds] objectAtIndex:indexPath.row];
    ...
    if (cell == nil)
    {
        cell = [[AllFeedsCustomCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier withIndex:indexPath.row dataMyFeed:myFeed isFromHome:YES];
    ...
    else if ([myFeed.vType_of_content isEqualToString:@"nowfriends"])
    {
        ...
        cell.lblWroteOn_Name.text = [NSString stringWithFormat:@"👥 %@",myFeed.fullname_iFriendsID];
    ...
    {
        cell.theFeedType=FEED_TYPE_WROTEHOME;
            [cell.imgViewPicture setImageWithURL:[NSURL URLWithString:myFeed.wrotedOn_image_path] placeholderImage:[UIImage imageNamed:@"sync.png"]];

        cell.btnViewPicture.tag =[myFeed.wroteOn_id integerValue];
        
        cell.btnViewPicture.userInteractionEnabled=(![myFeed.wroteOn_id isEqualToString:[[[NSUserDefaults standardUserDefaults]valueForKey:@"USER_DETAIL"]valueForKey:@"admin_id"]])?YES:NO;
        [cell.btnViewPicture addTarget:self action:@selector(btnViewProfileClicked:) forControlEvents:UIControlEventTouchUpInside];
        
        //cell.btnViewPicture.enabled=NO;
        
        cell.lblWroteOn_Name.text = [NSString stringWithFormat:@"%@",[self removeNull:myFeed.wrotedOn_Name]];
        cell.lblWroteOn_School.text = [NSString stringWithFormat:@"%@",[self removeNull:myFeed.wroteOn_School]];
        cell.lblWroteOn_NumOfFriend.text = [NSString stringWithFormat:@"%@ Friends",[self removeNull:myFeed.wroteOn_hasFriends]];
    }
    else if([myFeed.vType_of_content isEqualToString:@"birthdaywish"])
    {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
            ...
            || [myFeed.vType_of_content isEqualToString:@"badge"])
    {
        ...
        finalHeight=imageoriginy+(([myFeed.vType_of_content isEqualToString:@"badge"])?48:69);
    }
    else if([myFeed.vType_of_content isEqualToString:@"location"])
    {
...
-(void)tapGest:(UITapGestureRecognizer *)gest
{
    ...
    ViewFeed *myFeed = [[[MyAppManager sharedManager] arrAllFeeds] objectAtIndex:(long)gest.view.tag-1];
    if ([myFeed.vType_of_content isEqualToString:@"birthdaywish"])
    {
...
-(void)_deleteHandler:(NSString *)type :(NSString *)iActivityID
{
    ...
    else
    {
        ...
        NSString *strPostURL=[NSString stringWithFormat:@"userID=%@&vType_of_content=%@&iActivityID=%@",[[[NSUserDefaults standardUserDefaults]valueForKey:@"USER_DETAIL"]valueForKey:@"admin_id"],type,iActivityID];
...
{
    NSString *strMSG =[NSString stringWithFormat:@"%@",[[dictT objectForKey:@"MESSAGE"] removeNull]] ;
    if ([strMSG isEqualToString:@""])
    {
        return;
    }
