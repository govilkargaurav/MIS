...
@interface ResultAssSignViewController ()
{
    @private
    ...
    id<ResultAssSignViewControllerDelegate> delegate_;
}
...
@implementation ResultAssSignViewController
@synthesize
...
- (void)viewDidLoad
{
    ...
    aryTInfos = [[NSMutableArray alloc]initWithArray:[DatabaseAccess get_ResumeFTask:[NSString stringWithFormat:@"SELECT *FROM TBL_RESUMEFTASK WHERE cast(ResourceID as int) = %@ and ParticipantID  = '%@' and Status = 'Results'",globle_resource_id,globle_participant_id]]];
...
{
    /*----------------- Check Results [tbl_ResumeFTasks] Info in Database -----------------------*/
    
    
    if([aryTInfos count]>0) // Record Exits, So Update Info
    {
        NSString *strQuery = [NSString stringWithFormat:@"Update tbl_ResumeFTask set ResourceID = '%@',ParticipantID = '%@',Status = 'Results', PartName = '%@', AssName = '%@', AssDate = '%@' where AutoID = %@",globle_resource_id,globle_participant_id,[[aryParticipantInfoGlobal objectAtIndex:0] valueForKey:@"part_name"], tfAssName.text,pickDate.currentTitle,[[aryTInfos objectAtIndex:0] valueForKey:@"AutoID"]];
        [DatabaseAccess INSERT_UPDATE_DELETE:strQuery];
    }
    else                    // Record doesn't Exits, So Insert Info
    {
        NSString *strQuery = [NSString stringWithFormat:@"Insert into tbl_ResumeFTask (ResourceId,ParticipantID,AssessorID,Status,PartName,AssName,AssDate) values('%@','%@','%@','%@','%@','%@','%@')",globle_resource_id,globle_participant_id,[[NSUserDefaults standardUserDefaults] valueForKey:@"assempid"],@"Results",[[aryParticipantInfoGlobal objectAtIndex:0] valueForKey:@"part_name"],tfAssName.text,pickDate.currentTitle];
        [DatabaseAccess INSERT_UPDATE_DELETE:strQuery];
    }
